...
//    [Signal(detailed=true)]
    ...
    public async bool conversation_supports_reactions(Conversation conversation) {
        if (conversation.type_ == Conversation.Type.CHAT) {
            ...
            XmppStream? stream = stream_interactor.get_stream(conversation.account);
            if (stream == null) return false;
            bool? has_feature = yield stream_interactor.get_module(EntityInfo.IDENTITY).has_feature(conversation.account, conversation.counterpart, Xep.OccupantIds.NS_URI);
    ...
    private void on_account_added(Account account) {
        ...
        stream_interactor.module_manager.get_module(account, Xmpp.Xep.Reactions.Module.IDENTITY).received_reactions.connect((stream, jid, message_id, reactions, stanza) => {
            on_reaction_received.begin(account, stream, jid, message_id, reactions, stanza);
        });
    }

    private async void on_reaction_received(Account account, XmppStream stream, Jid jid, string message_id, Gee.List<string> reactions, MessageStanza stanza) {
        if (stanza.type_ == MessageStanza.TYPE_GROUPCHAT) {
            // We only accept reactions in MUCs if they support occupant ids
            bool? has_feature = yield stream_interactor.get_module(EntityInfo.IDENTITY).has_feature(account, jid, Xep.OccupantIds.NS_URI);
